...
/**
 * GraphStats prints a sorted list of Classes in a given Graph and the instance count of each.
 * 
...
public class GraphStats {	
	...
	public static void main(String[] argv) {
		if (argv.length < 1) die ("Please specify the Hypergraph location directory as parameter.");
	...
	public void printResults() {
		...
		for (Class<?> c : keys) {
			...
			System.out.println(df.format(i) + " " 
					+ df.format(classInstanceCounts.get(c)) 
					+ " " + (c == null? "No Java Type" : c.getSimpleName())
					+ " \t \t Total: " + df.format(accumulated)
					);
			i++;
		}
