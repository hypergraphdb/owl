...
public abstract class OntologyTransmitActivity extends FSMActivity {
	...
	public static String getDistributionModeFor(DistributedOntology dOnto) {
		if (dOnto instanceof PeerDistributedOntology) {
			...
			throw new IllegalStateException("Cannot determine valid distribution mode based on distributed ontology. Was:  " + dOnto);
		}
	}
	
	/**
	 * This will throw a meaningful Exceptions, if the push mode does not match the distributed ontology found on the target.
	 * @param dvoAtTarget the distributed ontology that exists on target
	 * @param distributionMode the distribution mode (requested by the client)
	 */
	protected void targetAssertDistributionModeMatches(DistributedOntology dvoAtTarget, String distributionMode) {
		if (distributionMode.equals(VALUE_DISTRIBUTION_MODE_CLIENT_SERVER)) {
			if (!(dvoAtTarget instanceof ServerCentralizedOntology)) {
				throw new RuntimeException(new VOWLSourceTargetConflictException("Target distributed ontology not appropriate for client/server distributionMode."));
			}
		} else if (distributionMode.equals(VALUE_DISTRIBUTION_MODE_PEER)) {
			if (!(dvoAtTarget instanceof PeerDistributedOntology)) {
				throw new RuntimeException(new VOWLSourceTargetConflictException("Target distributed ontology not appropriate for peer2peer distributionMode mode."));			}
		} else {
			throw new IllegalArgumentException("Distribution mode not recognized on target: " + distributionMode + " Only C/S and peer are available.");
		}
	}
