 ...
 * @created Oct 3, 2011
 * 
 *          2011.11.30 Optimizing this class (keep IRI typehandle) changed Load
 *          time of County ontology (1MB) from FunctionalSyntaxFile from 4m45sec
 *          to 1m45sec. 2011.12.01 Added default constructor, HG could not
 *          instantiate type.
 */
public class OWLNamedObjectType extends HGAtomTypeBase implements
		HGCompositeType
{

	@SuppressWarnings("unchecked")
	public static final List<Class<? extends OWLNamedObject>> OWL_NAMED_OBJECT_TYPES_HGDB = Arrays
			.<Class<? extends OWLNamedObject>> asList(
					OWLAnnotationPropertyHGDB.class, OWLClassHGDB.class,
					OWLDatatypeHGDB.class, OWLNamedIndividualHGDB.class,
					OWLDataPropertyHGDB.class, OWLObjectPropertyHGDB.class);
			...
			LazyRef<HGHandle[]> targetSet, IncidenceSetRef incidenceSet)
	{
		...
		{
			// TODO throw new
			// NullPointerException("IRI missing for OwlNamedObject at " +
			// handle);
		}
		//
		// using reflection
	...
	public HGPersistentHandle store(Object instance)
	{
		...
		return graph.getStore().store(
				new HGPersistentHandle[] { irihandle.getPersistent() });
	}
	...
	// 2011.11.30 OPTIMIZATION (Projection was created each time before, led to
	// loading class each time.)
	HGProjection projection = new HGProjection()
	{
		...
		public HGHandle getType()
		{
			if (typeHandle == null || !graph.isLoaded(typeHandle))
			{
				...
				// TODO 2011.12.07 Freeze early, so it does not happen during a
