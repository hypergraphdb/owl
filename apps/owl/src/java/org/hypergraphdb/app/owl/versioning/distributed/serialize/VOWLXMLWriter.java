...
public class VOWLXMLWriter extends OWLXMLWriter
{
	...
	{
		super(writer, vontology.ontology());
		startElementCount = 0;
		// we never use the superclass besides the forced super call.
		HGDBOntology ontology = vontology.ontology();
	...
	public void startDocument(OWLOntology ontology) throws OWLRendererException
	{
		...
		// if (!ontology.isAnonymous()) {
		// writer.writeAttribute(Namespaces.OWL + "ontologyIRI",
		// ontology.getOntologyID().getOntologyIRI().toString());
		// if (ontology.getOntologyID().getVersionIRI() != null) {
		// writer.writeAttribute(Namespaces.OWL + "versionIRI",
		// ontology.getOntologyID().getVersionIRI().toString());
		// }
		// }
		// }
		// catch (IOException e) {
