...
public class ChangeSetElementHandler extends AbstractVOWLElementHandler<ChangeSet>
{
	 ...
	 * .distributed.serialize.parse.VOWLChangeElementHandler)
	 */
	@Override
	public void handleChild(VOWLChangeElementHandler h) throws OWLXMLParserException
	{
		// We expect to be called in order here.
		// The first call to handleChild must refer to the oldest change in a
		// changeset.
	...
	{
		if (createdDate == null)
			throw new OWLParserException("Missing createdDate", getLineNumber(), getColumnNumber());
		if (changes == null)
			throw new OWLParserException("Missing changes", getLineNumber(), getColumnNumber());
	...
	{
		if (changeSet == null)
			throw new OWLXMLParserException("Expected Changeset was null", getLineNumber(), getColumnNumber());
		return changeSet;
	}
}