 ...
 * OWLLiteralHGDB.
 * 
 * Setters for literal and lang are provided for Hypergraph Bean persistence
 * only.
 * 
...
public class OWLLiteralHGDB extends OWLObjectHGDB implements OWLLiteral, HGLink
{
	...
	public OWLLiteralHGDB(HGHandle... args)
	{
		...
		// Don't call constructor, because we do not know at this point what
		// kind of literal we get. this("", args[0]);
		// And we cannot check for rdfPlainLiteralHandle as we do not have a
		// graph yet.
		if (args.length != 1)
	 ...
	 */
	public OWLLiteralHGDB(String literal, String lang,
			HGHandle rdfPlainLiteralHandle)
	{
		if (rdfPlainLiteralHandle == null)
	...
	{
		return getDatatype()
				.equals(getOWLDataFactory().getIntegerOWLDatatype());
	}
	...
	{
		return getDatatype()
				.equals(getOWLDataFactory().getBooleanOWLDatatype());
	}
