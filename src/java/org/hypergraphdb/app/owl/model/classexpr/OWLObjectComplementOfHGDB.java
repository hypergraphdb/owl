...
public class OWLObjectComplementOfHGDB extends OWLAnonymousClassExpressionHGDB implements OWLObjectComplementOf, HGLink
{
	...
	{
		// let's avoid late disaster, if entity is not a handle for a OWLEntity
		// subclass object.
		// We can;t do this here, because we have no graph yet!!!!
		// if (!TypeUtils.isClassAssignableFromAtomHandleType(OWLEntity.class,
		// owlEntityHandle, getHyperGraph())) {
		// throw new
		// IllegalArgumentException("OwlEntityHandle atom not usable as OWLEntity.");
		// }
		notifyTargetHandleUpdate(0, operand);
	}

	// public OWLObjectComplementOfHGDB(OWLDataFactory dataFactory,
